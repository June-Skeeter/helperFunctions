...
class typeEnforcer:
    ...
    def __post_init__(self):
        for (name, field_type) in self.__annotations__.items():
            ...
            if 'options' in self.__dataclass_fields__[name].metadata:
                if self.__dict__[name] not in self.__dataclass_fields__[name].metadata['options']:
                    self.error(f'{name} must be one of {self.__dataclass_fields__[name].metadata["options"]}')

    def error(self,msg='',ln=False,fn=False,kill=True):
        log(msg='',ln=False,fn=False,kill=True,verbose=self.verbose)
